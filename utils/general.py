...
def non_max_suppression(
    ...
    # Additional check to handle nested lists/tuples and ensure we have a tensor
    while isinstance(prediction, (list, tuple)):
        prediction = prediction[0]
    
    # Ensure prediction is a tensor
    if not isinstance(prediction, torch.Tensor):
        raise TypeError(f"Expected prediction to be a torch.Tensor, got {type(prediction)}")
